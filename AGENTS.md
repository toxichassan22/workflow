...
- Map preview «إعادة توليد» must actually fetch a new image: `POST /api/generate-map-image`
  ...
  `fonts/arabic-overlay-light.bin`; do not switch map labels to English.
...
`fonts/arabic-overlay-light.bin` (both are real fonts stored under non-LFS names). Hosting cannot
