...
المطلوب:
- اكتب تحليلًا موجزًا لكنه مكتمل التغطية في فقرتين إلى أربع فقرات، ولا تلتزم بعدد كلمات ثابت.
- غطِّ كل المعلومات المتاحة ذات الصلة: نوع المشروع، فكرته، وصفه، جمهوره المستهدف، مميزاته وفرصه الاستثمارية، طبيعة الموقع وموقعه الاستراتيجي، العنوان التفصيلي، الكثافة السكانية، البنية التحتية، الطرق، المعالم القريبة، معالم المدينة، نطاق التأثير، والمسافات وأوقات القيادة.
- اربط ملاءمة الموقع بنوع المشروع وفكرته والجمهور المستهدف، ولا تجعل التحليل مجرد قائمة مسافات.
- اجمع المعلومات المتشابهة في صياغة قصيرة بدون تكرار، وامنح كل معلومة متاحة وزنها المناسب حسب أهميتها.
- إذا كانت معلومة أساسية غير متاحة، لا تخمنها؛ تجاهلها أو اذكر باختصار أنها غير متوفرة.
...
{json.dumps(project_data, ensure_ascii=False, indent=2)}"""
    system_prompt = 'أنت محلل مواقع عقارية دقيق. أخرج تحليلًا عربيًا موجزًا مكتمل التغطية، يوازن بين جميع البيانات المتاحة دون تكرار أو اختلاق.'
    try:
        try:
