...
- **التصميم الأساسي الإلزامي**: تصميم جداول تقرير PDF المالي هو التصميم المعتمد والأساسي لجميع شرائح الدراسة المالية دون استثناء.
- **تطبيق ألوان الهوية**: لا تغيّر في الهيكل العام أو المساحات، وإنما تكيّف الألوان فقط لتتناسب مع هوية الشركة (اللون الأساسي للترويسات والحدود، نصوص داكنة مقروءة 4.5:1، وخلفيات خلايا نظيفة هادئة).
- **منع المربعات والكروت العائمة**: ممنوع منعاً باتاً تحويل جداول الدراسة المالية إلى كروت عائمة (cards) أو شبكة مربعات إحصائية (KPI boxes) أو تقسيم الشريحة إلى 4 خانات عائمة أو شريحة بها خانة واحدة. كل محتوى مالي يُعرض داخل جداول HTML نظامية كاملة (table).
- **جداول المؤشرات والملخصات (Key-Value Tables)**:
  - استخدم بنية جدول من عمودين (<table class="summary-table">): عمود البند/المؤشر بعرض 35%-40% مع خلفية هادئة بلون الهوية، وعمود القيمة بخط عريض bold وبأرقام منسقة بفواصل الآلاف.
  - عند وجود جدولين مترابطين (مثل «التكاليف والاستثمار» و«مؤشرات العائد والاسترداد»): رص الجدولين بجانب بعضهما في عمودين متجاورين (display:grid; grid-template-columns:1fr 1fr; gap:24px;) مع ترويسة أنيقة لكل جدول، بنفس فكرة ومساحات تقرير PDF.
- **جداول البيانات المتعددة الأعمدة (Data Tables)**:
  - استخدم <table class="data-table"> بكامل العرض (أو بجانب الرسم البياني المعتمد) مع صف عناوين مظلل بلون الهوية الأساسي وحدود واضحة 1px solid وفواصل آلاف للأرقام.
