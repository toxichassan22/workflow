...
## استمرارية سطح الشريحة — قاعدة تصميم غير قابلة للتجاوز
- جذر `slide` هو سطح بصري واحد متصل. لا تنشئ لوحة بيضاء كبيرة داخل شريحة ذات خلفية داكنة، ولا تضع المحتوى داخل مستطيل أبيض يترك إطارًا داكنًا حوله؛ هذا كسر للتسلسل البصري وليس بطاقة تصميم.
- إذا كانت خلفية جذر الشريحة داكنة، فتبقى كل منطقة المحتوى على السطح الداكن نفسه أو على درجات داكنة قريبة وشفافة، مع نص فاتح وحدود خفيفة. استخدم `background:transparent` أو `background:rgba(255,255,255,0.08)` للمساحات الداخلية، وليس `background:#ffffff` أو `#f8fafc` كلوحة حاملة.
- البطاقات والجداول على السطح الداكن تستخدم أسطحًا داكنة متدرجة أو شفافة، ولا تستخدم خلفية بيضاء كبيرة. يسمح بخلفية بيضاء فقط لحاوية شعار منفردة عندما يفرض تباين الشعار ذلك.
- عند تعديل شريحة موجودة، حافظ على `background` و`color` الخاصين بجذرها وعلى هندستها الأساسية. عدّل المحتوى المطلوب داخلها فقط، ولا تعِد بناء الشريحة بإطار أبيض أو لغة بصرية جديدة.
"""
...
- إذا كان جذر الشريحة داكنًا، يجب أن يندمج الهيدر مع سطح الجذر نفسه باللون الداكن، بلا شريط أبيض منفصل؛ يستخدم النص الفاتح والحدود الذهبية أو الفاتحة المقروءة.
...
## البطاقات (Cards)
تستخدم فقط لعنصرين أو ثلاثة مستقلين وقصيرين. على الخلفية الفاتحة: `background:#ffffff; color:{body_on_white}; border:1px solid #e2e8f0;`. على الخلفية الداكنة: `background:rgba(255,255,255,0.08); color:#f8fafc; border:1px solid rgba(255,255,255,0.22);`. لا تستخدم بطاقة بيضاء كبيرة كحامل لكل محتوى الشريحة، ولا تضع الجداول أو الفقرات المترابطة داخل لوحة بيضاء فوق Canvas داكن. استخدم `border-radius:10px; padding:12px 16px; box-sizing:border-box; box-shadow:none;`.
...
- **تطبيق ألوان الهوية والتناسب المضغوط (Compact & Fitted Tables)**:
  ...
  - **جداول العمودين والمؤشرات (2-Column Key-Value Tables)**:
    * عند احتواء الشريحة على جدول من عمودين فقط (مثل «البند» و«القيمة»): لا تمد الجدول بعرض 100% مفرط يباعد بين البند وقيمته؛ ضعه داخل حاوية أنيقة بعرض متناسب ومريح (max-width: 820px; margin: 0 auto; أو بعرض 65%-75% مريح) بحدود ناعمة. على السطح الفاتح يجوز استخدام بطاقة بيضاء، أما على السطح الداكن فاستخدم سطحًا داكنًا شفافًا ولا تنشئ لوحة بيضاء.
  ...
  - **جداول البيانات المتعددة الأعمدة (Data Tables)**:
    * استخدم table داخل حاوية متوافقة مع سطح الشريحة: بيضاء على السطح الفاتح، وداكنة أو شفافة على السطح الداكن، مع ترويسة أنيقة بلون الهوية الأساسي ({primary}) بلون نص أبيض ناصع، وصفوف متبادلة هادئة، وفواصل آلاف دقيقة للأرقام.
