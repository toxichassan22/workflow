...
## معالجة تباين الشعارات (Adaptive Logo Backgrounds)
- **تباين الشعارات الذكي:**
  - إذا كان الشعار (سواء شعار الشركة ##LOGO## أو شعار المشروع ##PROJECT_LOGO##) يحتوي على عناصر أو نصوص بيضاء أو فاتحة ويُعرض على خلفية بيضاء أو فاتحة، يجب وضعه داخل شارة بخلفية داكنة متناسقة (مثل: `background:{primary}; padding:6px 14px; border-radius:8px; display:inline-flex; align-items:center;`).
  - إذا كان الشعار داكناً ويُعرض على خلفية داكنة (مثل الغلاف أو الفوتر الداكن)، يجب وضعه داخل حاوية بيضاء أو فاتحة ناعمة (مثل: `background:#ffffff; padding:6px 14px; border-radius:8px; display:inline-flex; align-items:center;`).
...
## الرسوم البيانية عند الحاجة ومخططات التدفق للدراسة المالية (Flowcharts & Structured Tables)
- الشرائح المالية التي تتناول التدفقات، مراحل التطوير، سحب وسداد التمويل، أو هيكل الاستثمار: استخدم مخطط تدفق بصري هندسي راقٍ (Flowchart / Visual Pipeline) يربط المراحل والكتل بأسهم وخطوط وصل ومسارات تدفق بالألوان المؤسسية مع إبراز المبالغ لكل مرحلة.
- الشرائح المالية التي تتناول الجداول أو التكاليف أو المؤشرات: استخدم جداول بيانات احترافية منسقة بحدود ناعمة ورؤوس بارزة بوزن 700.
...
- الأرقام القابلة للمقارنة: رسم بياني مع جدول المصدر بجانبه.
- الصور والمخططات: صورة واحدة كبيرة أو صورتان بحد أقصى، مع عنوان ووصف كل صورة.
...
- شعار الشركة: ##LOGO## (height:36px في الهيدر، height:80px في الغلاف والختام، ضعه داخل شارة داكنة إذا كانت نصوصه بيضاء على خلفية فاتحة)
- شعار المشروع ##PROJECT_LOGO##: إذا ذُكر في «الصور المتوفرة» أنه متوفر فوضعه **إلزامي** — في هيدر كل شريحة محتوى بجانب شعار الشركة، وفي الغلاف والختام كذلك. الشعاران جنبًا إلى جنب بفاصل رأسي رقيق (1px solid #e2e8f0) وبارتفاع واحد متساوٍ (36px في الهيدر، 72px-80px في الغلاف والختام). استخدم شارة داكنة إذا كان أحدهما أبيض أو فاتح على خلفية فاتحة. إذا ذُكر أنه غير متوفر فلا تكتب ##PROJECT_LOGO## إطلاقًا.
...
## الرسوم البيانية ومخططات التدفق عند الحاجة
