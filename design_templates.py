...
- شريحة المود بورد والتصور الخارجي: شريحة واحدة فقط دائماً للمود بورد تجمع زوايا المشروع الأربع (##MOODBOARD_IMAGE_1##، ##MOODBOARD_IMAGE_2##، ##MOODBOARD_IMAGE_3##، ##MOODBOARD_IMAGE_4##) في شبكة 2x2 متناسقة مع التسميات.
- شرائح التصور الداخلي للمكونات: عدد شرائح التصور الداخلي = عدد المكونات المتوفرة (شريحة واحدة لكل مكون). كل شريحة تمثل مكوناً مستقلاً وتجمع كافة الصور المتوفرة لذلك المكون:
  * إذا كان للمكون صورة واحدة متولدة أو مرفوعة: اجعلها شريحة Hero كاملة ممتدة وبارزة مع تفاصيل وبيانات المكون (استخدم ##INTERIOR_COMP_1_IMG_1## أو ##INTERIOR_1_1##).
  * إذا كان للمكون أكثر من صورة (2 إلى 4 صور): اعرض كافة صور المكون مجتمعة داخل شريحة المكون في شبكة منتظمة (استخدم ##INTERIOR_COMP_1_IMG_1##، ##INTERIOR_COMP_1_IMG_2##، إلخ).
- المخططات المعمارية 2D: ##PLAN_IMAGE_1## إلى ##PLAN_IMAGE_4## (أو ##2D_PLAN_N##) في شرائح مساقط معمارية نظيفة ومميزة مع العنوان والشرح.
...
## الفواصل بين المحاور (section_divider)
- قبل كل محور رئيسي توجد شريحة فاصلة تحمل اسم المحور فقط، وخلفيتها هي الصورة الرئيسية المعتمدة (##IMAGE_COVER##) مع حجاب كحلي داكن فوقها ليظل النص مقروءًا.
- تخطيط الفاصل ثابت ويُبنى تلقائيًا: شريط {accent} رأسي على الحافة اليسرى، الشعار (والشعاران معًا إن وُجد شعار مشروع) أعلى اليسار، اسم المحور بالعربي كبيرًا وأبيض على اليمين، ترجمته الإنجليزية بلون {accent} تحته، خط أفقي قصير، ثم سطر وصفي واحد، ورقم الشريحة أسفل اليسار واسم المشروع أسفل اليمين.
- الفاصل بلا هيدر وبلا فوتر، وبلا بطاقات أو نقاط أو جداول. لا تُنشئ فاصلًا بنفسك في شرائح المحتوى.
...
- المخططات تُعرض **فقط** كصور مرفوعة من العميل عبر ##PLAN_IMAGE_1## إلى ##PLAN_IMAGE_4##، ووجودها إلزامي في الشرائح إن كانت متوفرة في «الصور المتوفرة».
- إذا لم تكن هناك مخططات مرفوعة فلا تُنشئ شريحة مخططات إطلاقًا.

## الرسوم البيانية عند الحاجة
- استخدم رسمًا بيانيًا (أعمدة أو دائري/Donut أو خط زمني) في أي شريحة يغلب عليها الأرقام: التكاليف، الإيرادات، التدفقات، التوزيع النسبي للمكونات، مقارنة المنافسين، مراحل التنفيذ.
- ارسمه بـ HTML/CSS فقط (أشرطة بارتفاعات نسبية، أو conic-gradient للدائري) بألوان الهوية، وبدون أي مكتبة رسم.
- كل رسم بياني يجب أن يكون مبنيًا على أرقام موجودة فعلًا في البيانات، وأن يظهر جدول الأرقام أو تسمياتها بجانبه.
